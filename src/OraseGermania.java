...
public class OraseGermania {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        while (scanner.hasNextLine())
        {
            ...
            if (valori.length > 7)
            if (!valori[7].isEmpty())
            if (Integer.parseInt(valori[7])>numarMinimLocuitori)
            {
                numeOrase.add(valori[0]);
                listaCoord.add(new Point2D.Double(Double.parseDouble(valori[2]),Double.parseDouble(valori[1])));
